...
async function bootstrap() {
  ...
  for (const local of configJson.locales) {
    if (
      !LANGUAGE_CODES.map((d) => d.toLocaleLowerCase()).includes(
        local.lang.toLocaleLowerCase() as LanguageCode
      )
    ) {
  ...
  for (const locale of configJson.locales) {
    ...
    try {
      ...
      if (
        ...
        loggerError(
          `Config type and file not match format. check docs more details`
        );
        process.exit(0);
      }
    } catch {
